...
const SEGMENT_SEO = {
  "interior-designing": ["Interior Designing — Atelier | BDSM · BDS Marvel", "Atelier, the interior designing team at BDSM · BDS Marvel: concept, spatial planning, material palettes and detailing for residential and commercial spaces."],
    ...
    <div className="network-panels">
      {segments.map((segment, index) => <article key={segment.slug} ref={(node) => (panelRefs.current[index] = node)} data-index={index} className={`network-panel network-panel--${segment.tone} ${index === active ? "network-panel--active" : ""} ${revealed[index] ? "network-panel--revealed" : ""}`} data-testid={`network-panel-${index + 1}`}>
        ...
        <div className="network-panel__copy" ref={(node) => (copyRefs.current[index] = node)} data-copy-index={index}>
          <div className="network-panel__brand"><span className="np-brand-parent">{segment.slug === "interior-designing" ? "BDS MARVEL" : segment.parent}</span><span className="np-brand-name">{segment.name}</span></div>
