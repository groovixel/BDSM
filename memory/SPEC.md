...
- Stays & booking: `/stays` page lists three BnB tiers — Premium `The Living Studio` (Alibaug, ₹28,000/night), Mid `The Material Loft` (Jaipur, ₹12,500/night), Budget-friendly `The Courtyard Room` (Kishangarh, ₹5,800/night) — each with tier badge, description, amenities and a BOOK THIS STAY button opening a booking-request modal (name, email, check-in/out dates, guests, notes); and email the owner; overlapping date ranges per stay are blocked only for `confirmed` bookings (HTTP 409); new bookings start as `requested` and are shown in the modal as "awaiting confirmation" without blocking, via `GET /api/stays/availability`; the owner flips status with `POST /api/bookings/{id}/status` (`requested`/`confirmed`/`cancelled`). All "Request a Night" CTAs (homepage spotlight + experience page) now route to `/stays`, and `/experience` gained a "Three ways to stay" tier-card section.