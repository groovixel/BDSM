...
- Stays & booking: `/stays` is a category overview (Premium / Mid / Budget-friendly); each category has its own page `/stays/premium`, `/stays/mid`, `/stays/budget-friendly` listing five BnBs each (15 stays total, placeholder names/locations/prices) with tier badge, description, amenities and a BOOK THIS STAY button opening a booking-request modal (name, email, check-in/out dates, guests, notes); requests persist to Mongo `bookings` and email the owner; overlapping date ranges per stay are blocked only for `confirmed` bookings (HTTP 409); new bookings start as `requested` and are shown in the modal as "awaiting confirmation" without blocking, via `GET /api/stays/availability`; the owner flips status with `POST /api/bookings/{id}/status` (`requested`/`confirmed`/`cancelled`). All "Request a Night" CTAs (homepage spotlight + experience page) route to `/stays`, and `/experience` tier cards link to each category page.